...
public class Run_exp extends Thread {
	...
	public static void main(String[] args)
	{
	    ...
	    // code review metrics 
		/*
	    cols.put("author_experience",1); 
		cols.put("author_merge_ratio",1); 
		cols.put("author_changes_per_week",1); 
		cols.put("author_merge_ratio_in_project",1); 
		cols.put("total_change_num",0); 
		cols.put("author_review_num",0); 
		cols.put("description_length",0); 
		cols.put("is_documentation",0); 
		cols.put("is_bug_fixing",0); 
		cols.put("is_feature",0); 
		...
		cols.put("avg_reviewer_experience",1); 
		cols.put("avg_reviewer_review_count",1); 
		...
		*/
		...
		String TARGET = "bug" ; 
		int nruns = 5 ;
		String datapath = "C:/Users/Motaz/Desktop/work/DP_performance_complexity/data/xerces-1.3"  ; //"C:/Users/AQ38570/Desktop/test_data" ; 
		String result_path = "C:/Users/Motaz/Desktop/work/DP_performance_complexity/res_trials/xerces-1.3" ; 
		...
		for (int i = 0; i < listOfFiles.length; i++)
		{
			if (listOfFiles[i].getName().contains("train")) 
			{
				...
				for (int run = 0 ; run < nruns ; run++) {
					...
					Run_MOGP ibea_run = new Run_MOGP(train,cols,TARGET,"ibea",multi_metrics,100,1000,0.9,0.1,new ArrayList<Solution>()) ; 
					Run_MOGP simple_nsga3_run = new Run_MOGP(train,cols,TARGET,"nsga3",multi_metrics,1200,3200,0.7,0.5
							,new ArrayList<Solution>()) ; 
					
					Run_MOGP extended_nsga3_run = new Run_MOGP(train,cols,TARGET,"nsga3",ibea_metrics,1000,400,0.7,0.3,new ArrayList<Solution>()) ; 
